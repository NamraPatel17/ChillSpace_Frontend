...
export default function HostLayout() {
  ...
  const [hostProfile, setHostProfile] = useState({ fullName: "", profilePicture: "" });
            ...
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center flex-shrink-0">
                {hostProfile.profilePicture ? (
                  <img src={hostProfile.profilePicture} alt={hostProfile.fullName} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-xs font-semibold text-gray-700">{getInitials(hostProfile.fullName)}</span>
                )}
              </div>
